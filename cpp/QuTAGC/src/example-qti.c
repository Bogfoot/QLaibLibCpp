 ...
 *
 *******************************************************************************/
/* $Id: example0.c,v 1.3 2021/02/03 13:40:14 trurl Exp $ */

#include <stdio.h>
#include <string.h>
#include <stdlib.h>              /* for exit() */
#include "tdcbase-qti.h"
 ...
 */
static void run( int selftest, int flatgen, int normgen, int ch1, int ch2 )
{
  ...
  /*
   * To enable external clock use the following code:
   */
#ifndef USE_EXTERNAL_CLOCK
  Bln32 locked;
  TDC_getClockState(&locked);
  if ( !locked )
    TDC_enableExternalClock(1);
  // Check if clock is locked
  SLEEP(3000);
  TDC_getClockState(&locked);
  printf(">>> Clock is %s locked\n", locked ? "" : "not ");
#endif  // USE_EXTERNAL_CLOCK

  bin2ns = HIST_BINWIDTH * timeBase * 1.e9;            /* Width of a bin in nanoseconds */
  rc = TDC_enableChannels( 1, 0xffffffff );            /* Use all channels */
  ...
  for ( i = 0; i < 10; ++i ) {
    printf( ">>> " );
    for ( j = 0; j < 5; j++ ) {
      int ch = channels[5*i+j];
      ch += ch < 100 ? 1 : 0;                          /* "human readable" channel numbers */
      printf( "%"LLXFORMAT" (%d)  ", timestamps[5*i+j], ch );
    }
    printf( "\n" );
  }
  printf( "\n" );
  ...
  printf( ">>>                    starts=%d  stops=%d expTime=%g s\n", eventsA, eventsB,
          expTime * timeBase );
  rc = TDC_getHistogram( ch1, ch2, 1, hist2, &count, &tooSmall, &tooBig, &eventsA, &eventsB, &expTime );
  checkRc( "TDC_getHistogram", rc );
  printf( ">>> Histogram %d-%d:     valid=%d tooSmall=%d tooBig=%d\n", ch1, ch2, count, tooSmall, tooBig );
  printf( ">>>                    starts=%d  stops=%d expTime=%g s\n", eventsA, eventsB,
          expTime * timeBase );
  printf( ">>>       Bin Time   Counter global      Counter %d-%d\n", ch1, ch2 );
...
{
  int signal = 0, selftest = 0, flatgen = 0, normgen = 0, ch1 = 1, ch2 = 2;
  if ( argc > 1 ) {
  ...
  if ( (argc != 2 && argc != 4) || !(signal || selftest || normgen || flatgen) ) {
    printf( "usage: %s [signal | selftest | flatgen | normgen] {<ch1> <ch2>}\n", argv[0] );
    return 1;
  }
  run( selftest, flatgen, normgen, ch1, ch2 );
  return 0;
}
